(function () {
  ...
  var lineLB = function lineLB(csvFile, cities) {
    ...
    function setupScales() {
      var countX = d3.scaleTime().domain([2010, 2020]);
    ...
    function drawAxes(g) {
      ...
      g.select('.axis-x').attr('transform', "translate(0,".concat(height, ")")).transition().duration(450).ease(d3.easeLinear).call(axisX);
      var axisY = d3.axisLeft(scales.count.y).tickPadding(5).tickFormat(d3.format('d')).tickSize(-width).ticks(6);
      g.select('.axis-y').transition().duration(450).ease(d3.easeLinear).call(axisY);
    }
  ...
  var scatterLB = function scatterLB(csvFile, cities) {
    ...
    function setupScales() {
      var countX = d3.scaleLinear().domain([d3.min(filteredData, function (d) {
        return d.percentage;
      }), d3.max(filteredData, function (d) {
        return d.percentage;
      })]);
      var countY = d3.scaleLinear().domain([d3.min(filteredData, function (d) {
    ...
    function updateChart(dataz) {
      ...
      layer.merge(newLayer).attr('cx', function (d) {
        ...
        tooltip.style('opacity', 1).html("".concat(tooltipHeader, "\n            <p class=\"tooltip-scatter-text\">Poblaci\xF3n en ").concat(firstYear, ": <strong>").concat(d.populationFirstYear, "</strong><p/>\n            <p class=\"tooltip-scatter-text\">Poblaci\xF3n en ").concat(secondYear, ": <strong>").concat(d.populationSecondYear, "</strong><p/>")).style('left', postionWidthTooltip > w ? positionright : positionleft).style('top', "".concat(d3.event.pageY - 100, "px"));
      }).on('mouseout', function () {
        tooltip.transition().duration(200).style('opacity', 0);
      }).attr('fill-opacity', 0.8).transition().duration(450).ease(d3.easeLinear).attr('cy', function (d) {
    ...
    function menuFirstYear() {
      var selectFirstYear = d3.select("#select-first-year-".concat(cities));
      selectFirstYear.selectAll('option').data(years).enter().append('option').attr('value', function (d) {
        return d;
      }).text(function (d) {
        return d;
      });
    ...
    function menuSecondYear() {
      var selectSecondYear = d3.select("#select-second-year-".concat(cities));
      selectSecondYear.selectAll('option').data(years).enter().append('option').attr('value', function (d) {
        return d;
      }).text(function (d) {
        return d;
      });
    ...
    function loadData() {
      d3.csv(csvFile).then(function (data) {
        ...
        menuFirstYear();
        menuSecondYear();
        d3.select("#select-second-year-".concat(cities)).property('value', secondYear);
        updateYearData();
      });
    }
    ...
    function updateYearData() {
      var mergeYears = [];
      var dataFirstYear = filterDataByYear(+firstYear, 'populationFirstYear');
      var dataSecondYear = filterDataByYear(+secondYear, 'populationSecondYear');
      mergeYears = dataFirstYear.map(function (item, i) {
        return Object.assign({}, item, dataSecondYear[i]);
      });
      filteredData = mergeYears.map(function (_ref) {
        var populationFirstYear = _ref.populationFirstYear,
            populationSecondYear = _ref.populationSecondYear,
            name = _ref.name,
            cp = _ref.cp;
        var difference = populationSecondYear - populationFirstYear;
        return {
          percentage: +(difference * 100 / populationFirstYear).toFixed(2),
          populationFirstYear: populationFirstYear,
          populationSecondYear: populationSecondYear,
          cp: cp,
          name: name
        };
      });
      lossPopulation = filteredData.filter(function (_ref2) {
        var percentage = _ref2.percentage;
        return percentage < 0;
      }).length;
      winPopulation = filteredData.filter(function (_ref3) {
        var percentage = _ref3.percentage;
        return percentage > 0;
      }).length;
      d3.select(".municipios-lost-".concat(cities)).remove();
      d3.select(".municipios-wins-".concat(cities)).remove();
      svg.select(".scatter-lb-".concat(cities, "-container")).append('text').attr('class', "municipios-lost-".concat(cities)).attr('y', '95%').attr('x', '5%').text("Un total de ".concat(lossPopulation, " municipios han perdido poblaci\xF3n."));
      svg.select(".scatter-lb-".concat(cities, "-container")).append('text').attr('class', "municipios-wins-".concat(cities)).attr('y', '91%').attr('x', '5%').text("Un total de ".concat(winPopulation, " municipios han ganado poblaci\xF3n."));
    ...
    function filterDataByYear(yearFilter, position) {
      var dataYearsFiltered = dataz.filter(function (_ref4) {
        var year = _ref4.year;
        return year === yearFilter;
      });
      return dataYearsFiltered.map(function (_ref5) {
        var _ref6;

        var population = _ref5.population,
            municipio = _ref5.municipio,
            cp = _ref5.cp;
        return _ref6 = {}, _defineProperty(_ref6, position, population), _defineProperty(_ref6, "name", municipio), _defineProperty(_ref6, "cp", cp), _ref6;
      });
    }

    d3.select("#select-compare-".concat(cities)).on('click', function (dataz) {
      updateYearData();
    });
    window.addEventListener('resize', resize);
    loadData();
